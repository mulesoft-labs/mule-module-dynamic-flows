...
/**
 * AddMessageProcessor invokes the {@link org.mule.module.dynamicFlows.DynamicFlowsModule#add(java.lang.String, java.util.List)} method in {@link DynamicFlowsModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private List<String> _configsType;
    private static Logger logger = LoggerFactory.getLogger(AddMessageProcessor.class);
    /**
     * Module object
     * 
     */
     ...
     * @param value Value to set
     */
    public void setConfigs(Object value) {
        this.configs = value;
    }
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("add"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((DynamicFlowsModuleLifecycleAdapter) moduleObject);
        }
        try {
            String transformedContextName = ((String) evaluateAndTransform(muleMessage, AddMessageProcessor.class.getDeclaredField("_contextNameType").getGenericType(), contextName));
            List<String> transformedConfigs = ((List<String> ) evaluateAndTransform(muleMessage, AddMessageProcessor.class.getDeclaredField("_configsType").getGenericType(), configs));
            retryCount.getAndIncrement();
            castedModuleObject.add(transformedContextName, transformedConfigs);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("add"), event, e);
        }
    }
